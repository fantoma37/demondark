...
const roleGroups = [
  { title: 'Good team', roles: [['Priest', 'Learn if your chosen player is good. The Fool counts as not good.'], ['Sage', 'Tomorrow night, learn the role of your chosen player.'], ['Guard', 'Your chosen player cannot die tonight.'], ['Knight', 'If you die, and your chosen player is the Demon, they die instead.']] },
...
function App() {
  ...
  if (state.room.phase === 'resolution' && state.room.resolution) return <main className="shell resolution-screen"><Header status="RESOLUTION" onGuide={openGuide} /><div className="resolution-card"><div className="resolution-mark">{state.room.resolution.kind === 'execution' ? '⚖' : '☾'}</div><p className="eyebrow">The table has spoken</p><h1>{state.room.resolution.title}</h1><p>{state.room.resolution.copy}</p><strong className="resolution-countdown">{remaining}</strong></div><RoleGuide open={guideOpen} onClose={closeGuide} /></main>
  ...
  return <main className={`shell game-shell ${state.room.phase}`}><Header status={isNight ? 'NIGHT' : 'DAY'} time={remaining} onGuide={openGuide} /><div className="game-intro"><p className="eyebrow">{isNight && me.alive ? `You are the ${me.role}` : me.alive ? 'Day phase' : 'You are dead'}</p><h2>{displayedRole}</h2>{isNight && state.teamInfo?.bluff && <div className="bluff-note"><span>Your bluff</span><strong>{state.teamInfo.bluff}</strong></div>}{isNight && state.teamInfo && <p className="team-note">Your allies: {state.teamInfo.allies.join(' and ')}</p>}</div><section className="action-area">{canAct && (isNight ? targets.length > 0 : true) ? <><div className="action-heading"><span>{isNight ? 'Choose one player' : 'Vote to execute'}</span><small>{isNight ? 'Your choice is private' : 'Tap your vote again to retract it'}</small></div><div className="target-grid">{targets.map((player) => <button className={`target ${selected === player.id ? 'selected' : ''}`} key={player.id} onClick={() => choose(player.id)}><span className="avatar">{player.nickname.slice(0, 1)}</span><span>{player.nickname}{player.id === me.id ? ' (you)' : ''}</span><i>{selected === player.id ? '✓' : '○'}</i></button>)}</div><div className="action-explainer">{isNight ? (me.immediateInfo || state.nightPrompt) : 'Choose yourself or another player. Select your vote again to remove it.'}</div></> : <div className="sleep-state"><span className="moon">☾</span><h3>{me.role === 'Demon' && state.room.firstNight ? 'The first night is for learning.' : me.alive ? 'Your choice is locked in.' : 'Your story has ended.'}</h3><p>{me.alive ? (me.immediateInfo || 'Stay quiet. Watch the clock.') : 'You can still watch the village.'}</p></div>}</section><div className="alive-strip">{state.room.players.map((player) => <span className={player.alive ? '' : 'is-dead'} key={player.id}>{player.nickname.slice(0, 1)}</span>)}</div>{state.room.hostId === socket.id && state.room.phase === 'day' && <button className="end-day" onClick={() => socket.emit('endDay', { code: roomCode })}>End day early</button>}<RoleGuide open={guideOpen} onClose={closeGuide} /></main>
}
