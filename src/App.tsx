...
const roleGroups = [
  ...
  { title: 'Wild card', roles: [['Fool', 'You have no night action. If the town executes you, you win.']] }
]
...
function App() {
  ...
  return <main className={`shell game-shell ${state.room.phase}`}><Header status={isNight ? 'NIGHT' : 'DAY'} time={remaining} onGuide={openGuide} /><div className="game-intro"><p className="eyebrow">{isNight && me.alive ? `You are the ${me.role}` : me.alive ? 'Day phase' : 'You are dead'}</p><h2>{displayedRole}</h2>{isNight && me.roleDescription && <p className="ability-copy">{me.roleDescription}</p>}{isNight && state.teamInfo?.bluff && <div className="bluff-note"><span>Your bluff</span><strong>{state.teamInfo.bluff}</strong></div>}{isNight && state.teamInfo && <p className="team-note">Your allies: {state.teamInfo.allies.length ? state.teamInfo.allies.join(' and ') : 'none'}</p>}</div><section className="action-area">{canAct && (isNight ? targets.length > 0 : true) ? <><div className="action-heading"><span>{isNight ? 'Choose one player' : 'Vote to execute'}</span><small>{isNight ? 'Your choice is private' : 'Votes are private'}</small></div><div className="target-grid">{targets.map((player) => <button className={`target ${selected === player.id ? 'selected' : ''}`} key={player.id} onClick={() => choose(player.id)}><span className="avatar">{player.nickname.slice(0, 1)}</span><span>{player.nickname}{player.id === me.id ? ' (you)' : ''}</span><i>{selected === player.id ? '✓' : '○'}</i></button>)}</div><div className={`action-explainer ${me.immediateInfo ? 'information-copy' : ''}`}>{isNight ? (me.immediateInfo || state.nightPrompt) : 'Choose yourself or another player.'}</div></> : <div className="sleep-state"><span className="moon">☾</span><h3>{me.role === 'Demon' && state.room.firstNight ? 'The first night is for learning.' : me.alive ? 'Your choice is locked in.' : 'Your story has ended.'}</h3><p>{me.alive ? (me.immediateInfo || 'Stay quiet. Watch the clock.') : 'You are still allowed to talk in the village.'}</p></div>}</section><div className="alive-strip">{state.room.players.map((player) => <span className={player.alive ? '' : 'is-dead'} key={player.id}>{player.nickname.slice(0, 1)}</span>)}</div>{state.room.hostId === socket.id && state.room.phase === 'day' && <button className="end-day" onClick={() => socket.emit('endDay', { code: roomCode })}>End day early</button>}<RoleGuide open={guideOpen} onClose={closeGuide} /></main>
}
