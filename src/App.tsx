...
const roleGroups = [
  { title: 'Good team', roles: [['Priest', 'Learn if your chosen player is good.'], ['Sage', 'Tomorrow night, learn the role of your chosen player.'], ['Guard', 'Your chosen player cannot die tonight.'], ['Knight', 'If you die, and your chosen player is the Demon, they die instead.'], ['Bard', 'The player you choose learns that you are the Bard.']] },
...
function App() {
  ...
  return <main className={`shell game-shell ${state.room.phase}`}><Header status={isNight ? 'NIGHT' : 'DAY'} time={remaining} onGuide={openGuide} /><div className="game-intro"><p className="eyebrow">{isNight && me.alive ? `You are the ${me.role}` : me.alive ? 'Day phase' : 'You are dead'}</p><h2>{displayedRole}</h2>{isNight && me.immediateInfo && <div className="information-copy top-information">{me.immediateInfo}</div>}{isNight && me.roleDescription && <p className="ability-copy">{me.roleDescription}</p>}{isNight && state.teamInfo?.demon && <div className="evil-info">Your Demon is <strong>{state.teamInfo.demon}</strong></div>}{isNight && state.teamInfo?.minion && <div className="evil-info">Your Minion is <strong>{state.teamInfo.minion}</strong></div>}{isNight && state.teamInfo?.bluff?.map((bluff) => <div className="bluff-note" key={bluff}><span>Bluff</span><strong>{bluff}</strong></div>)}</div><section className="action-area">{canAct && (isNight ? targets.length > 0 : true) ? <><div className="action-heading"><span>{isNight ? 'Choose one player' : 'Vote to execute'}</span><small>{isNight ? 'Your choice is private' : 'Votes are private'}</small></div><div className="target-grid">{targets.map((player) => <button className={`target ${selected === player.id ? 'selected' : ''}`} key={player.id} onClick={() => choose(player.id)}><span className="avatar">{player.nickname.slice(0, 1)}</span><span>{player.nickname}{player.id === me.id ? ' (you)' : ''}</span><i>{selected === player.id ? '✓' : '○'}</i></button>)}</div></> : <div className="sleep-state"><span className="moon">☾</span><h3>{me.role === 'Demon' && state.room.firstNight ? 'First night.' : me.alive ? 'Choice locked.' : 'You are dead.'}</h3><p>{me.alive ? 'Waiting.' : 'You are still allowed to talk in the village.'}</p></div>}</section><div className="alive-strip">{state.room.players.map((player) => <span className={player.alive ? '' : 'is-dead'} key={player.id}>{player.nickname.slice(0, 1)}</span>)}</div>{state.room.hostId === socket.id && state.room.phase === 'day' && <button className="end-day" onClick={() => socket.emit('endDay', { code: roomCode })}>End day early</button>}<RoleGuide open={guideOpen} onClose={closeGuide} /></main>
}
